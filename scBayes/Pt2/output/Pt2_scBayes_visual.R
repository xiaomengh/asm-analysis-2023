...
########## plot all myeloid cells from three timepoint together
...
for (sampleid in c("T1","T2","T3")){
  ...
  if(sampleid=="T1"){
    ...
    assign$Barcode = gsub("-1",".1",assign$Barcode)
    assign = fillInQual(assign)
    assign[assign$QUAL<14,]$ASIG = "UNASSIGN"
  }
  if(sampleid=="T2"){
    assign=read.table(paste(sampleid,".post.assign",sep=""),sep='\t',header=T)
    colnames(data) = gsub(".1",".2",colnames(data))
    assign$Barcode = gsub("-1",".2",assign$Barcode)
    assign = fillInQual(assign)
    assign[assign$QUAL<14,]$ASIG = "UNASSIGN"
  }
  if(sampleid=="T3"){
    assign=read.table(paste(sampleid,".post.assign",sep=""),sep='\t',header=T)
    colnames(data) = gsub(".1",".3",colnames(data))
    assign$Barcode = gsub("-1",".3",assign$Barcode)
    assign = fillInQual(assign)
    assign[assign$QUAL<14,]$ASIG = "UNASSIGN"
  }
...
dev.off()
########## plot all lymphocytes cells from three timepoint together
