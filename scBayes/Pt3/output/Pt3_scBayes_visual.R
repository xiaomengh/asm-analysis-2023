plotCellVariantMatrix = function(data,mainlabel,cells,variant_range=NA){
  ...
  c = df_heatmap[df_heatmap$value>0.1,]
...
plotQualityHeatmap=function(assignment, cells, minlim=5, maxlim=150) {
  if(sum(assignment$QUAL>maxlim)>0) {
    assignment[assignment$QUAL>maxlim,]$QUAL = maxlim
  }
  ...
  ggplot(qual_reorder, aes(x = Barcode,y=variable)) +
...
plotCellVariantBySubcloneSortByQualAndByCellType = function(cells, celltype,is.all,assignment){
  ...
  sc4_cell_index = colnames(ordered_data) %in% cells & (colnames(ordered_data) %in% sc4)
  ...
  sc4_data = ordered_data[,sc4_cell_index]
  ...
  print("sc4 cell number")
  print(sum(sc4_cell_index))
  print("normal cells")
  ...
  if (is.all){
    new_data = cbind(ordered_data$variant,sc1_data,sc2_data,sc3_data,sc4_data,normal_data,unassign_data)
  } else {
    new_data = cbind(ordered_data$variant,sc1_data,sc2_data,sc3_data,sc4_data,normal_data)
  }
  ...
  abline(v = sum(sc1_cell_index)+sum(sc2_cell_index)+sum(sc3_cell_index)+sum(sc4_cell_index)+0.5)#when include normal
  if (is.all) {
    abline(v = sum(sc1_cell_index)+sum(sc2_cell_index)+sum(sc3_cell_index)+sum(sc4_cell_index)+ncol(normal_data)+0.5)
  }
}
...
########## plot myeloid cells: subclones + normal #########
...
for (sampleid in c("T1","T2","T3")){
  ...
  if(sampleid=="T1"){
    colnames(data) = gsub(".1",".4",colnames(data))
    assign=read.table(paste(sampleid,".pre.assign",sep=""),sep='\t',header=T)
    assign = fillInQual(assign)
    assign[assign$QUAL<5,]$ASIG = "UNASSIGN"
    assign$Barcode = gsub("-1",".4",assign$Barcode)
  }
  if(sampleid=="T2"){
    colnames(data) = gsub(".1",".5",colnames(data))
    assign=read.table(paste(sampleid,".post.assign",sep=""),sep='\t',header=T)
    assign = fillInQual(assign)
    assign[assign$QUAL<6,]$ASIG = "UNASSIGN"
    assign$Barcode = gsub("-1",".5",assign$Barcode)
  }
  if(sampleid=="T3"){
    colnames(data) = gsub(".1",".6",colnames(data))
    assign=read.table(paste(sampleid,".post.assign",sep=""),sep='\t',header=T)
    assign = fillInQual(assign)
    assign[assign$QUAL<6,]$ASIG = "UNASSIGN"
    assign$Barcode = gsub("-1",".6",assign$Barcode)
  }
  ...
  sc4 = assign[assign$ASIG=='SC4',]$Barcode
